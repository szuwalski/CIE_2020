...
all_dat$model<-rep(c("F35 projection","5 year average of recent F"),each=nrow(in_dat))
...
# organize data_2019
repfile<-scan("19_gmacs_proj/mcoutSSB.REP")
hist_ssb<-matrix(repfile,ncol=37,byrow=T)
colnames(hist_ssb)<-seq(1982,2018)

innames<-c("Draw","Replicate","Treatment",paste("F",seq(1,length(M[[1]]$log_fbar))),
           "B35",paste("proj_ssb_",seq(2019,2025)))
projfile<-as.data.frame(matrix(scan("19_gmacs_proj/mcoutPROJ.REP"),ncol=length(innames),byrow=TRUE))
colnames(projfile)<-innames
...
# organize data_2020
repfile20<-scan("20_gmacs_proj/mcoutSSB.REP")
hist_ssb20<-matrix(repfile20,ncol=length(seq(1982,2019)),byrow=T)
colnames(hist_ssb20)<-seq(1982,2019)

innames<-c("Draw","Replicate","Treatment",paste("F",seq(1,length(M[[1]]$log_fbar))),
           "B35",paste("proj_ssb_",seq(2020,2025)))
projfile20<-as.data.frame(matrix(scan("20_gmacs_proj/mcoutPROJ.REP"),ncol=length(innames),byrow=TRUE))
colnames(projfile20)<-innames
...
png("plots/projection_F35.png",height=5,width=8,res=400,units='in')
print(both_proj)
dev.off()
